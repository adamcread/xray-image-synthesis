...
class objComposeUnsuperviseModel(BaseModel):
    ...
    def forward_inpainting(self):
        ...
        self.mask_A1_T = (self.mask_A1_T<0.9*torch.max(self.mask_A1_T).data).type(torch.FloatTensor).to(self.device)
        self.mask_A2_T = (self.mask_A2_T<0.9*torch.max(self.mask_A2_T).data).type(torch.FloatTensor).to(self.device)
    ...
    def forward_test(self):
        ...
        self.mask_A1_T = (self.mask_A1_T<self.opt.Thresh1*torch.max(self.mask_A1_T).data).type(torch.FloatTensor).to(self.device)
        self.mask_A2_T = (self.mask_A2_T<self.opt.Thresh2*torch.max(self.mask_A2_T).data).type(torch.FloatTensor).to(self.device)
        ...
        self.real_M0_s = 1 - (((self.mask_A1_T + self.mask_A2_T)>=1)*1).type(torch.FloatTensor).to(self.device)
        ...
        missed_overlp = (((self.real_M1_s + self.real_M2_s)==0)*1).type(torch.FloatTensor).to(self.device)
    ...
    def forward(self):
        ...
        self.real_A1 = Variable(self.input_B1) # img 1
        self.real_A2 = Variable(self.input_B2) # img 2
        self.real_B = Variable(self.input_B) # unpaired image

        # real transformation that has occurred
    ...
    def backward_STN(self):
        ...
        self.loss_STN = 50*(self.criterionFocalTversky(self.stn_B2_T, self.real_B2_T))
        self.loss_STN += 50*(self.criterionFocalTversky(self.stn_B2, self.real_B2))
    ...
    def backward_G(self):
        ...
        # pixel L1 loss
        self.loss_G_L1 = 0.5*(self.criterionFocalTversky(self.fake_B1_T, self.real_A1_T.detach()) +
                          self.criterionFocalTversky(self.fake_B2_T, self.real_A2_T.detach()))
        self.loss_G_L1 += self.criterionL1(self.fake_B, self.real_B)
        self.loss_G_L1 += 0.5*(self.criterionFocalTversky(self.fake_A1_T, self.real_A1_T.detach()) +
